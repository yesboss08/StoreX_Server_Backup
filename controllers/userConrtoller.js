...
const {userData}  = req
const dbData = await userModels.findById(userData?.userId).lean()
return res.status(200).json({name:dbData?.name , email:dbData?.email, emailVarified:dbData?.emailVarified, role:dbData?.role, totalStorage:dbData.maxStorageInBytes,isSubscribed:dbData?.purchasedPlan?.PlanStatus=="active"?true:false})  
}
